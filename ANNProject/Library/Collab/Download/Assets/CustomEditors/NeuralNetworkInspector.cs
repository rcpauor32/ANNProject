...
public class NeuralNetworkWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        PrintNetworkInfo();
        // Adding ---
        EditorGUI.BeginDisabledGroup(network.state == NeuralNetwork.NetworkState.Training);
    ...
    void PrintNetworkInfo()
    {
        ...
        if (network != null)
            network.state =
                (NeuralNetwork.NetworkState)EditorGUILayout.EnumPopup("Network Mode:", network.state, GUILayout.ExpandWidth(false));
    }
    ...
    private void ProcessContextMenu(Vector2 mousePos)
    {
        ...
        if (network.state == NeuralNetwork.NetworkState.Design)
        {
...
public class GraphicConnection
{
    ...
    public void Draw()
    {
        ...
        Handles.DrawBezier(
            ...
            color,
            null,
            1 + (ANNMathHelpers.Sigmoid(connection.weight) - 0.5f) * ANNProperties.GraphicWeightMultiplier
            );
    }
}
#endregion
